...
/****************************************************************
    added by makeblock fftust at 2017/3/27, just used as a test to
    use ADC2 CHANNELS
****************************************************************/
static esp_err_t adc2_pad_get_io_num(adc2_channel_t channel, gpio_num_t *gpio_num)
{
...
static esp_err_t adc2_pad_init(adc2_channel_t channel)
{
    ...
    ADC1_CHECK_FUNCTION_RET(adc2_pad_get_io_num(channel, &gpio_num));
...
{
    portENTER_CRITICAL(&rtc_spinlock);
    SET_PERI_REG_BITS(SENS_SAR_START_FORCE_REG, SENS_SAR2_BIT_WIDTH_V, width_bit, SENS_SAR2_BIT_WIDTH_S);  //SAR2_BIT_WIDTH[1:0]=0x3, SAR1_BIT_WIDTH[1:0]=0x3
    ...
    SET_PERI_REG_BITS(SENS_SAR_READ_CTRL_REG, SENS_SAR2_SAMPLE_BIT_V, width_bit, SENS_SAR2_SAMPLE_BIT_S); //digital sar1_bit_width[1:0]=3
    portEXIT_CRITICAL(&rtc_spinlock);
...
int adc2_get_voltage(adc2_channel_t channel)
{
    ...
    SET_PERI_REG_BITS(SENS_SAR_MEAS_START2_REG, 1, 1, SENS_SAR2_EN_PAD_FORCE_S); //open the ADC1 data port
    //Select channel
...
/****************************************************************
    added by makeblock fftust at 2017/3/27, just used as a test 
****************************************************************/
