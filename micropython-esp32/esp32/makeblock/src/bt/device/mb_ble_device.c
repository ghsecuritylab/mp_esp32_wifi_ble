 ...
 ******************************************************************************/
mb_ringbuf_t ble_data_rb;
uint8_t slot_data_cnt;
uint8_t slot_data_buf[BLE_RING_BUF_SIZE];
SemaphoreHandle_t slot_data_sem = NULL;
...
static void mk_uart_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param )
{
    ...
    switch (event) {
    	...
    	case ESP_GATTS_WRITE_EVT: 
			...
			for ( idx = 0; idx < param->write.len; idx++ ) {
				ESP_LOGI( GATTS_TABLE_TAG, "%02x", param->write.value[idx] );
			  	mb_ringbuf_put( &ble_data_rb, param->write.value[idx] );
			}
      	break;
    	...
    	case ESP_GATTS_CONNECT_EVT:
			...
			ESP_LOGI( GATTS_TABLE_TAG, "pyexec switch to ble mode\n" );
			pyexec_set_ctrl_mode( CTRL_MODE_BLE );
        	break;
			
    	case ESP_GATTS_DISCONNECT_EVT:
			// to do re-advertising
			ESP_LOGI( GATTS_TABLE_TAG, "Peer device disconnect, re-advertising\n" );
			ESP_LOGI( GATTS_TABLE_TAG, "pyexec switch to uart mode\n" );
			pyexec_set_ctrl_mode( CTRL_MODE_UART );
			esp_ble_gap_start_advertising(&heart_rate_adv_params);
		break;
...
void mb_ble_device_init( void )
{
	...
	// ringbuf inti
	mb_ringbuf_alloc( (&ble_data_rb), BLE_RING_BUF_SIZE );

	slot_data_sem = xSemaphoreCreateMutex();
	if ( NULL == slot_data_sem ) {
		ESP_LOGE( tag, "Cann't create mutex for slot data\n" );
		while( 1 );
	}
...
/******************************************************************************/
int mb_get_ble_char( void )
{
	return mb_ringbuf_get( &ble_data_rb );
}

int mb_push_to_ble( uint8_t v )
{
	int ret;
	
	if ( slot_data_cnt < BLE_RING_BUF_SIZE ) {
		printf( "%02x", v );
		slot_data_buf[slot_data_cnt++] = v;
		if ( slot_data_cnt >=2 && 0x0d == slot_data_buf[slot_data_cnt-2] && 0x0a == slot_data_buf[slot_data_cnt-1] ) {
			// send to ble master
			esp_ble_gatts_send_indicate( mk_uart_profile_tab[0].gatts_if, mk_uart_profile_tab[0].conn_id, mk_uart_handle_table[MK_IDX_RX_CHAR_VAL],
				                         slot_data_cnt, slot_data_buf, false );
			slot_data_cnt = 0;
		}
		ret = 0;
	}
